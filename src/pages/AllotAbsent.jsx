...
const initialMappings = [
  { studentName: 'John Doe', volunteerName: 'Jane Smith', approved: false, allotted: false, preferredVolunteer: 'ABC' },
  { studentName: 'Alice', volunteerName: 'Robert Brown', approved: false, allotted: false, preferredVolunteer: 'XYZ' },
...
const AllotAbsent = () => {
  ...
  const handleManualAllot = (e) => {
    ...
    const newMapping = { studentName: studentName.value, volunteerName: volunteerName.value, approved: false, allotted: true };
  ...
  return (
    <div className="flex justify-center items-center min-h-screen bg-gray-50">
      <div className="bg-white rounded-lg shadow-md p-6 w-full max-w-4xl">
        <h3 className="text-xl font-semibold mb-4 text-center">Volunteer Allotment in case of absence</h3>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 justify-center mb-4">
          {mappings.map((mapping, index) => (
            <div key={index} className="bg-gray-100 p-4 rounded shadow-md">
              <span className="font-semibold">Student:</span> {mapping.studentName}
              <br />
              <span className="font-semibold">Absent Volunteer:</span> {mapping.volunteerName}
              <br />
              <span className="font-semibold">Preferred Volunteer:</span> {mapping.preferredVolunteer || 'None'}
              <div className="mt-2">
                {mapping.allotted ? (
                  <button
        ...
        {manualAllot && (
          <div className="mt-6 bg-gray-100 p-4 rounded shadow-md">
            <h4 className="text-lg font-semibold">Manual Allotment for {rejectedStudent}</h4>
